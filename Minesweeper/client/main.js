...
import { countSolutions, solver } from './solver_main.js';
import { formatSolutions } from './solver_main.js';
...
let analysisMode = true; // * GDMEM CHANGED THIS
...
export async function startup() {
    ...
    // window.BINOMIAL = new Binomial(70000, 500);
    ...
    await newGame(width, height, mines, 0); // * seed = 0
    gameBoard = board;
    ...
    console.log("Welcome to minesweeper solver dedicated to Annie");
}
...
async function saveMBF(e) {
    ...
    if (analysisMode) {
        ...
        if (mbf == null) {
            console.log("No game data available to convert to an MBF file");
            okay = false;
        }
    }
...
async function playAgain() {
    ...
    if (board != null && !analysisMode) {
        ...
        console.log("Replay game requested");
    } else {
        console.log("No game to replay");
    }
...
async function newGameFromBlob(blob) {
    ...
    console.log("Game " + board.width + "x" + board.height + "/" + board.num_bombs + " created from MBF file " + blob.name);
}
...
async function newGameFromMBF(mbf) {
    ...
    //console.log("Game "  + width + "x" + height + "/" + mines + " created from MBF file");
 
}

async function newBoardFromFile(inputString) {
    console.log("got into newboard fromfile!");
    ...
    console.log("Position loaded!");
        ...
        //     if (mbf == null) {
        //         console.log("File " + file.name + " doesn't contain data for a whole board");
        //         return;
        //     } else {
        //         newGameFromMBF(mbf);
        //         console.log("Game " + board.width + "x" + board.height + "/" + board.num_bombs + " created from mine positions extracted from file " + file.name);
        //     }
        // }
...
function loadReplayData(file) {
    ...
    if (!analysisMode) {
        console.log("Switch to analysis mode before loading the replay");
        return;
    }
    ...
    fr.onloadend = async function (e) {
        ...
        console.log("Replay for " + replayData.header.width + "x" + replayData.header.height + "/" + replayData.header.mines + " loaded from " + file.name);
...
async function newGame(width, height, mines, seed) {
    ...
    // console.log("New game requested with width " + width + ", height " + height + " and " + mines + " mines.");
...
async function replayForward(replayType) {
    ...
    while (replayStep != size) {
        ...
        if (replayType == "S") {
            console.log("Advancing to step " + replayStep + " of " + size);
            await sleep(1);
        }
...
async function replayBackward(replayType) {
    ...
    while (replayStep != 0) {
        ...
        if (replayType == "S") {
            console.log("Backwards to step " + replayStep + " of " + size);
            await sleep(1);
        }
...
export async function doAnalysis() {
    ...
    let compressed = "";
    // if (docAnalysisParm.value == "full") { // * Default is "none" but idk what this does
    ...
    // if (compressed.length < 2000) { // * Don't think this matters
    ...
    if (solutionCounter.finalSolutionsCount != 0) {
        ...
        // show the next tile to be clicked if in replay mode // * Not relevant to what I'm doing
...
async function checkBoard() {
    ...
    if (solutionCounter.finalSolutionsCount != 0) {
        ...
        // analysisButton.disabled = true;
        //console.log("The board is in an invalid state. " + msg + " " + board.getFlagsPlaced() + " Mines placed. ");
        ...
        if (button == 1) {   // left mouse button
            ...
            if (!tile.isCovered()) {  // clicking on a revealed tile is considered chording
                if (board.canChord(tile)) {
                    ...
                    if (docHardcore.checked) {
                        ...
                        if (!lethalChord && uncertainChords.length > 0 && board.hasSafeTile()) {
                            ...
                            console.log("Hard Core: Game is lost because you guessed (by chording) when there were safe tiles!");
                ...
                if (docHardcore.checked && tile.getHasHint() && tile.probability != 1 && tile.probability != 0 && board.hasSafeTile()) {
                    ...
                    console.log("Hard Core: Game is lost because you guessed when there were safe tiles!");
...
function prefixMessage(text) {
    if (messageLine.innerHTML != "") {
        console.log(text + " - " + messageLine.innerHTML);
    } else {
        console.log(text);
    }
